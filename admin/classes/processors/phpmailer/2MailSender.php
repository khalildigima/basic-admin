...
class MailSender
{
	...
	function send()
	{
		...
		try {
			...
			$mail->Host       = 'mail.smptpserver.name';                 // Set the SMTP server to send through
			$mail->SMTPAuth   = true;                                   // Enable SMTP authentication
			$mail->Username   = 'user@name.email';               // SMTP username
			...
			//Recipients
			$mail->setFrom('user@name.email', 'CFMS');
	...
	{
		$this->from = "contact@vincitalents.in";
		$this->to = $email;
		$this->subject = "Vinci Talents - Email verification";
	...
	function email_verification_message($email)
	{
		...
		$link = 'http:/' . '/vincitalents.in/verify_email.php?email=' . $email . '&token=' . $token;
	...
	function password_recovery_mail($email)
	{
		$this->from = "contact@vincitalents.in";
		$this->to = $email;
		$this->subject = "Vinci Talents - Password Reset";
	...
	function password_reset_message($email)
	{
		...
		$link = 'http:/' . '/vincitalents.in/password_reset.php?email=' . $email .  '&token=' . $token;
